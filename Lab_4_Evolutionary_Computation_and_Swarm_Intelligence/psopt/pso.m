...
if isscalar(options.InertiaWeight)
    state.ParticleInertia = options.InertiaWeight ; 
else
    state.ParticleInertia = options.InertiaWeight(1); % Initial inertia
end
...
for k = 1:options.Generations
    ...
    % Update Velocity and Diversity histories
    % ---------------------------------------------------------------------
    state.VelocityMag = sqrt(sum(state.Velocities.^2,2));
    state.Diversity = sum(var(state.Population));
    ...
    if ~isempty(options.PlotFcns) && ~mod(k,options.PlotInterval)
        ...
        for i = 1:length(options.PlotFcns)
            ...
            if iscell(options.PlotFcns)
                if iscell(options.PlotFcns{i})
                    state = options.PlotFcns{i}{1}(options,state,flag,options.PlotFcns{i}{2});
                else
