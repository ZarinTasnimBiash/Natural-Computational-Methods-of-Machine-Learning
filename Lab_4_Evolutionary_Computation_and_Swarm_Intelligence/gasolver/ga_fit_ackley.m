function [f] = ObjFun_Ackley(population, bounds, param)
% ObjFun_Ackley Computes the negated value of Ackley's function
% Values are negated so that a maximizing solver can minimize the function.
% Inputs:
...
f = -20*exp(-sqrt(sum(population(1:popsize,:).^2, 2)/dim)/5) - exp(sum(cos(2*pi*population(1:popsize,:)), 2)/dim) + exp(1) + 20;
f = f';
